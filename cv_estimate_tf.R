...
cv_estimate_tf <- function(
    ...
    lambda = double(50), #%% How many nsol and lambda by default? 20, 50, 100? Should be the same?
    lambda_max = -1,
    maxiter = 1e3L,
    regular_splits = TRUE, #%% False is default in cv_estimate_rt https://github.com/dajmcdon/rtestim/blob/main/R/cv_estimate_rt.R. Though easier for us to use TRUE.
    invert_splits = FALSE,
    ...) {
  ...
  for (f in 1:nfold) {
    ...
    x_hat <- matrix(NA, nrow = n, ncol = length(lambda))
